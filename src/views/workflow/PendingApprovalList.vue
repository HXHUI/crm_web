<template>
  <div class="pending-approval-list">
    ...
    <ContractDetailDialog
      v-if="currentInstance?.businessType === 'contract'"
    ...
    <!-- 订单详情对话框 -->
    <OrderDetailDialog
      v-if="currentInstance?.businessType === 'order'"
...
const currentContractId = ref<string | number>()
...
const loadBusinessNumber = async (instance: WorkflowInstance) => {
  ...
  try {
    if (instance.businessType === 'quote') {
      ...
      const response = await orderApi.getDetail(String(instance.businessId)) as unknown as {
        code: number
...
const loadPendingApprovals = async () => {
  try {
    loading.value = true
    const response = await getMyPendingApprovals(pagination.page, pagination.pageSize) as unknown as {
      code: number
      data?: {
...
const viewDetail = async (instance: WorkflowInstance) => {
  ...
  if (instance.businessType === 'quote') {
    ...
    currentContractId.value = instance.businessId
    contractDetailDialogVisible.value = true
  } else if (instance.businessType === 'order') {
    currentOrderId.value = instance.businessId
...
const handleApprovalSuccess = () => {
  ...
  if (currentInstance.value) {
    if (quoteDetailDialogVisible.value && currentInstance.value.businessType === 'quote') {
      viewDetail(currentInstance.value)
    } else if (contractDetailDialogVisible.value && currentInstance.value.businessType === 'contract') {
      viewDetail(currentInstance.value)
    } else if (orderDetailDialogVisible.value && currentInstance.value.businessType === 'order') {
      viewDetail(currentInstance.value)
    }
  }
}
