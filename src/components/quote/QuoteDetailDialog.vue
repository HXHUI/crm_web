<template>
  ...
  >
    <div v-if="quote" class="detail-layout">
      <!-- 左侧导航按钮 -->
      ...
      <div class="right-content">
        ...
        <div class="dynamic-content-section detail-scroll-container" ref="detailContentRef" @scroll="handleDetailScroll">
          <!-- 基本信息 -->
          <el-card id="quote-section-basic" shadow="never" class="section-card basic-info-section detail-section">
            <template #header>
              <div style="display: flex; justify-content: space-between; align-items: center">
                <h3 class="section-title" style="margin: 0">基本信息</h3>
                <div class="action-buttons">
                  <el-button
                    v-if="quote.status === 'draft' || quote.status === 'rejected'"
                    type="primary"
                    :icon="Edit"
                    @click="handleEdit"
                  >
                  ...
                  >
                    {{ quote.status === 'rejected' ? '重新提交审批' : '提交审批' }}
                  </el-button>
                </div>
              </div>
            </template>
          ...
          <!-- 审批流程 -->
          <el-card id="quote-section-approval" shadow="never" class="section-card tab-content detail-section">
            <template #header>
              <div style="display: flex; justify-content: space-between; align-items: center">
                <h3 class="section-title" style="margin: 0">审批流程</h3>
                <div
                  v-if="quote.status === 'pending_approval' && approvalInstance && canApprove(approvalInstance)"
            ...
            <div v-else-if="quote.status === 'draft' || quote.status === 'rejected'" class="no-approval">
              <el-empty :description="quote.status === 'rejected' ? '审批已被驳回' : '尚未提交审批'" />
    ...
    <div v-else class="loading-container">
      <el-empty description="加载报价详情失败，请刷新重试" />
    </div>
...
import quoteApi, { type Quote } from '@/api/quote'
import {
  getQuoteApprovalInstance,
  submitQuoteApproval,
...
const scrollToSection = (tab: string) => {
  nextTick(() => {
    const sectionId = `quote-section-${tab}`
...
const handleDetailScroll = () => {
  ...
  const sections = [
    { id: 'quote-section-basic', tab: 'basic' },
    { id: 'quote-section-approval', tab: 'approval' },
  ]
...
const loadQuoteDetail = async () => {
  ...
  try {
    loading.value = true
    const response = await quoteApi.getDetail(String(props.quoteId)) as unknown as {
      code: number
      data?: Quote
    }
...
const loadApprovalInstance = async () => {
  ...
  try {
    const response = await getQuoteApprovalInstance(Number(props.quoteId)) as unknown as {
      code: number
...
const submitApprovalDirectly = async (templateId: number, submitComment?: string) => {
  ...
  try {
    ...
    const response = await submitQuoteApproval(Number(quote.value.id), {
      templateId,
      submitComment,
...
const handleSubmitApproval = async () => {
  if (!submitApprovalFormRef.value || !quote.value) return
...
onMounted(() => {
  if (props.modelValue && props.quoteId) {
    loadQuoteDetail()
  }
})
</script>
