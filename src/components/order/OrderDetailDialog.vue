<template>
  ...
  >
    <div v-if="order" class="detail-layout">
      ...
      <div class="right-content">
        ...
        <div class="dynamic-content-section detail-scroll-container" ref="detailContentRef" @scroll="handleDetailScroll">
          <!-- 基本信息 -->
          <el-card id="order-section-basic" shadow="never" class="section-card basic-info-section detail-section">
            <template #header>
              <div style="display: flex; justify-content: space-between; align-items: center">
                ...
                <div class="action-buttons">
                  ...
                  >
                    {{ order.status === 'rejected' ? '重新提交审批' : '提交审批' }}
                  </el-button>
                </div>
              </div>
            </template>
          ...
          <!-- 审批流程 -->
          <el-card id="order-section-approval" shadow="never" class="section-card tab-content detail-section">
            <template #header>
              <div style="display: flex; justify-content: space-between; align-items: center">
                <h3 class="section-title" style="margin: 0">审批流程</h3>
                <div
                  v-if="order.status === 'pending_approval' && approvalInstance && canApprove(approvalInstance)"
            ...
            <div v-else-if="order.status === 'draft' || order.status === 'rejected'" class="no-approval">
              <el-empty :description="order.status === 'rejected' ? '审批已被驳回' : '尚未提交审批'" />
    ...
    <div v-else class="loading-container">
      <el-empty description="加载订单详情失败，请刷新重试" />
    </div>
...
const scrollToSection = (tab: string) => {
  nextTick(() => {
    const sectionId = `order-section-${tab}`
...
const handleDetailScroll = () => {
  ...
  const sections = [
    { id: 'order-section-basic', tab: 'basic' },
    { id: 'order-section-approval', tab: 'approval' },
  ]
...
const loadOrderDetail = async () => {
  ...
  try {
    loading.value = true
    const response = await orderApi.getDetail(String(props.orderId)) as unknown as {
      code: number
      data?: Order
    }
...
const loadApprovalInstance = async () => {
  ...
  try {
    const response = await getOrderApprovalInstance(Number(props.orderId)) as unknown as {
      code: number
...
const submitApprovalDirectly = async (templateId: number, submitComment?: string) => {
  ...
  try {
    ...
    const response = await submitOrderApproval(Number(order.value.id), {
      templateId,
      submitComment,
...
const handleSubmitApproval = async () => {
  if (!submitApprovalFormRef.value || !order.value) return
